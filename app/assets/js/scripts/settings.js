    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    'Uwaga!<br>To Twoje Ostatnie Konto',
                    'Żeby używać launchera musisz być zalogowany do co najmniej jednego konta. Będziesz musiał zalogować się ponownie.<br><br>Czy napewno chcesz się wylogować?',
                    'Wyloguj',
                    'Zakończ'
                )
                setOverlayHandler(() => {
            ...
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">Nazwa Użytkownika</div>
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Wybrane Konto &#10004;' : '>Wybierz Konto'}</button>
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Nie udało się usunąć<br> Dodatkowej Modyfikacji ${fullName}`,
                    'Sprawdź czy plik nie jest w użyciu i spróbuj ponownie.',
                    'Ok'
                )
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            'Nie udało się włączyć<br>Jednej lub Wiecej Dodatkowych Modyfikacji',
                            err.message,
                            'Ok'
                        )
                        setOverlayHandler(null)
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Wybrano: Java ${v.version.major} Update ${v.version.update} (x${v.arch})`
            } else {
                settingsJavaExecDetails.innerHTML = `Wybrano: Java ${v.version.major}.${v.version.minor}.${v.version.revision} (x${v.arch})`
            }
        } else {
            settingsJavaExecDetails.innerHTML = 'Nieprawidłowy Wybór'
        }
    })
}
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        ...
        titleElement.innerHTML = 'Oficjalne Wydanie - Copyright © 2017-2021 Ciochinio'
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/Etternal-red/BastionLauncher/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = 'Nie udało się załadować opisów wersji.'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `Nowa ${isPrerelease(data.version) ? 'Pre-Aktualizacja' : 'Aktualizacja'} Dostępna`
        ...
        settingsUpdateButtonStatus('Szukaj Aktualizacji', false, () => {
            if(!isDev){
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Wyszukiwanie Aktualizacji...', true)
            }
        })
    }
}
